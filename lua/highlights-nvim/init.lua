...
local function load_palette(name)
  local cs = M.config.colorschemes[name]
  if not cs or not cs.palette then
    return nil
  end
  ...
  vim.notify(string.format("highlights-nvim: Failed to load palette for %q: %s", name, result), vim.log.levels.WARN)
  return nil
end
...
local function resolve_color(group, hl, palette)
  ...
  if resolved.fg == false then
    resolved.fg = nil
  elseif resolved.fg then
    resolved.fg = resolve_attr(resolved.fg, "fg")
  end

  if resolved.bg == false then
    resolved.bg = nil
  elseif resolved.bg then
    resolved.bg = resolve_attr(resolved.bg, "bg")
  end
